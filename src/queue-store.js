  ...
  try {
    ...
    for (const item of items) {
      store.put({
        ...
        relativePath: String(item.relativePath || '').slice(0, 1024),
        folderSegments: Array.isArray(item.folderSegments) ? item.folderSegments.map((segment) => String(segment || '').trim()).filter(Boolean).slice(0, 64) : [],
        folderBaseParent: Number.isSafeInteger(item.folderBaseParent) && item.folderBaseParent >= 0 ? item.folderBaseParent : 0,
        folderBasePath: Array.isArray(item.folderBasePath) && item.folderBasePath.length ? item.folderBasePath.slice(0, 64) : ['根目录'],
        status: item.status === 'done' || item.status === 'error' ? item.status : 'ready',
        error: item.error || '',
